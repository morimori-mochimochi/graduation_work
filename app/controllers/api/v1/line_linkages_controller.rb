  ...
  module V1
    ...
    class LineLinkagesController < BaseController
      ...
      def new
        ...
        response = line_messaging_client.issue_link_token(user_id: current_user.id)
      ...
      rescue Line::Bot::Api::ApiError => e
        Rails.logger.error "Failed to create link token: #{e.class} - #{e.message} - #{e.response.body if e.respond_to?(:response)}"
      ...
      def callback
        ...
        events.each do |event|
          ...
          if event.is_a?(Line::Bot::Event::AccountLink)
            ...
            line_messaging_client.push_message(user.line_messaging_user_id, message)
          end
        end
