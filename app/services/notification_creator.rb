...
class NotificationCreator
  ...
  def self.call(save_route:, user:); end
  ...
  def call
    ...
    unless start_datetime_present?
      return Result.new(false, nil,
                        { alert: I18n.t('notifications.create.start_time_blank') })
    end

    notification = @save_route.notifications.find_or_initialize_by(user: @user)
    notification.assign_attributes(
      notify_at: self.class.calculate_notify_at(@save_route.execution_date, @save_route.start_time), status: 'pending'
    )

    if notification.save
      message = { notice: I18n.t('notifications.create.notice',
