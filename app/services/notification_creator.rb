...
class NotificationCreator
  def initialize(save_route:, user:, set_to: nil)
    @save_route = save_route
    @user = user
    @set_to = set_to || 'email' # デフォルトはemail
  end

  # 結果を返すのみの構造体
  Result = Struct.new(:success?, :notification, :message)

  def self.call(save_route:, user:, set_to: nil)
    new(save_route: save_route, user: user, set_to: set_to).call
  end

  def call
    return handle_missing_datetime unless start_datetime_present?
    return hadle_missing_line_uid if line_notification_without_uid?
  ...
  def assign_notification_attributes(notification)
    ...
    notification.assign_attributes(notify_at: notify_at, status: 'pending', set_to: @set_to)
  end
  ...
  def line_notification_without_uid?
    @set_to.to_s == 'line' && @user.line_login_uid.blank?
  end
end
